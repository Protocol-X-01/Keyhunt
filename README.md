...
You need to add `-t numberThreads` to get better speeds
...
You need to add `-t numberThreads` and `-k factor` to get better speeds
...
for the legacy version also you are going to need:
...
```
git clone https://github.com/OMNIP01/keyhunt.git
```
...
btw any words/characters after the publickey will be ignored as the file can only be the publickeys:
...
When we run the program for the second time the files will be read and the bP Points processing is omitted:
...
All the next examples were made with the `-S` option I just omit that part of the output to avoid confusions use `-S` if you want, but remember with a large `-n` there must also come massive files
...
We can speed up our process selecting a bigger K value `-k value` btw the n value is the total length of item tested in the random range, a bigger k value means more ram to be used:
...
Note the value of N `0xfffff000000` with k = 20 this means that the N value is less than the default value `0x100000000000` that is because k is not a 2^X number
...
### Valid n and maximum k values for specific 
...
**If you exceed the max value of K the program may exhibit unexpected behavior, the program may have suboptimal performance, or in other cases you can miss some collisions or have an incorrect speed.**

Note for users that want to use it with SWAP memory. IT DOESN'T WORK with Swap Memory was made to small chucks of memory also is slowly.   
...
To check this I prepared a set of test publickeys to be found at some specific time according to your speed.
...
The minikey are generated from a 16 byte buffer using the base58 encode function using the bitcoin string `123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz` any suggestions please let me know.

The input file can be made of addresses or rmd hashlist of the target keys
...
I already explained the speed for BSGS
...
of each thread is finished the code updates its own counter in 1

```
steps[thread_number]++;
```

So every step represents 1024 keys scanned.

if you enabled endomorphism, the total steps are multiplied by 6 for modes `address`, `rmd160` and `vanity`.
Because with endomorphism we’re checking, effectively, 6 different keys every step
 
For `xpoint` mode plus endomorphism the number is only is multiplied by 3 only because we only care
...
because we are checking effectively 2 keys the program calculates one X value and 
it is checking both prefixes `02 + X value` and `03 + X value`, this is NOT optional. If you try to do it without this behavior the speed will be worse

This is important because if you targeting a specific range with `compress` and WITHOUT endomorphism e.g for the puzzles
 the effective will be half of the speed displayed by the program
But if you are targeting all of the curve then the speed displayed will be correct.
