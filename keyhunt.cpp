...
// random_tracker: replaced by FeistelPermutation (see below)
...
// Random key tracking functions
// Feistel-based random tracker (see end of file)
...
// ─── Feistel Permutation Engine ─────────────────────────────────────────────
// Replaces the std::set<std::string> random tracker with a bijective
// permutation that visits every key in the range exactly once in a
// pseudo-random order, using O(1) memory.
//
// Analogy: shuffle a deck of N cards perfectly, then deal them one by one.
// The Feistel cipher is a bijection by construction, so every index maps
// to a unique output — no repeats, no misses, no memory needed.
// ────────────────────────────────────────────────────────────────────────────

// Feistel round function: mix half-block with round key
static inline uint64_t feistel_round(uint64_t half, uint64_t round_key, int half_bits) {
	// SplitMix64-style mixing for good avalanche
	uint64_t x = half ^ round_key;
	x ^= x >> 30;
	x *= 0xbf58476d1ce4e5b9ULL;
	x ^= x >> 27;
	x *= 0x94d049bb133111ebULL;
	x ^= x >> 31;
	return x & ((1ULL << half_bits) - 1);
}

struct FeistelPermutation {
	uint64_t domain_size;     // The range size (n_range_end - n_range_start)
	int total_bits;           // ceil(log2(domain_size))
	int half_bits_L;          // Left half bit width
	int half_bits_R;          // Right half bit width
	uint64_t mask_L;
	uint64_t mask_R;
	uint64_t round_keys[8];  // 8 rounds of Feistel
	int num_rounds;

	// Global counter — next index to permute
	uint64_t counter;
	bool exhausted;
...
static FeistelPermutation *g_feistel = NULL;

// Forward-permute a single index through the Feistel network
// Returns a value in [0, 2^total_bits), not necessarily in [0, domain_size)
static uint64_t feistel_encrypt(const FeistelPermutation *fp, uint64_t index) {
	uint64_t L = (index >> fp->half_bits_R) & fp->mask_L;
	uint64_t R = index & fp->mask_R;

	for (int i = 0; i < fp->num_rounds; i++) {
		uint64_t new_L = R & fp->mask_L;  // Truncate R to L's width
		uint64_t F = feistel_round(R, fp->round_keys[i], fp->half_bits_L);
		uint64_t new_R = (L ^ F) & fp->mask_R;
		L = new_L;
		R = new_R;
	}
	return (L << fp->half_bits_R) | R;
}

// Cycle-walking: re-encrypt until the output lands in [0, domain_size)
static uint64_t feistel_permute(const FeistelPermutation *fp, uint64_t index) {
	uint64_t result = feistel_encrypt(fp, index);
	// Cycle-walk: if result is outside domain, encrypt again
	// This terminates because the Feistel is a permutation on a finite set
	while (result >= fp->domain_size) {
		result = feistel_encrypt(fp, result);
	}
	return result;
}

#if defined(_WIN64) && !defined(__CYGWIN__)
HANDLE feistel_mutex_handle;
#else
pthread_mutex_t feistel_mutex_handle;
#endif
...
int main(int argc, char **argv)	{
	...
	// Apply KFACTOR to CPU_GRP_SIZE for all modes
	if(KFACTOR > 1) {
		CPU_GRP_SIZE = CPU_GRP_SIZE_BASE * KFACTOR;
		// Ensure CPU_GRP_SIZE is reasonable (not too large to cause memory issues)
	...
	while ((c = getopt(argc, argv, "deh6MqRSDB:b:c:C:E:f:I:k:l:m:N:n:p:r:s:t:v:G:8:z:")) != -1) {
		switch(c) {
			case 'h':
				menu();
			break;
			...
			case 'R':
				...
				FLAGRANDOM = 1;
				FLAGBSGSMODE =  3;
				// init_random_tracker() is called after range setup below
			break;
			case 'r':
				if(optarg != NULL)	{
	...
	if(FLAGMODE != MODE_BSGS )	{
		...
		// Initialize Feistel permutation AFTER ranges are set
		if (FLAGRANDOM) {
			init_random_tracker();
		}
	...
	fclose(fd);
		...
		// Re-init Feistel now that BSGS ranges are set
		if (FLAGRANDOM) {
			init_random_tracker();
		}
	...
	M3	5497558139
		*/
		...
		for(uint32_t i = 2; i < CPU_GRP_SIZE / 2; i++) {
...
void *thread_process(void *vargp)	{
	...
	do {
		if(FLAGRANDOM){
			if(!generate_unique_random_key(key_mpz, n_range_start, n_range_end)) { continue_flag = 0; }
		}
		else	{
...
#endif
					pts[0] = pn;
					for(uint32_t i = 0; i<CPU_GRP_SIZE && bsgs_found[k]== 0; i++) {
...
void *thread_process_bsgs_random(void *vargp)	{
	...
	*/
		...
		if(!generate_unique_random_key(base_key, n_range_start, n_range_end)) break;
#if defined(_WIN64) && !defined(__CYGWIN__)
		ReleaseMutex(bsgs_thread);
#else
		pthread_mutex_unlock(&bsgs_thread);
...
void *thread_process_bsgs_dance(void *vargp)	{
	...
	*/
	...
	// Use proper random generation instead of problematic rand() % 3
	if(BSGS_CURRENT.IsLower(&n_range_end))	{
		if(!generate_unique_random_key(base_key, BSGS_CURRENT, n_range_end)) break;
...
void menu() {
	...
	printf("-D          Disable random mode (force sequential mode)\n");
	...
	printf("-k value    K factor for performance boost. In BSGS mode: factor for M. In other modes: multiplies CPU_GRP_SIZE. More speed but more RAM usage\n");
	...
	printf("-R          Enable random mode (default is sequential)\n");
...
// ─── Feistel Permutation: "Shuffle the deck" implementation ─────────────────
// Every key in the range is visited exactly once, in pseudo-random order.
// O(1) memory. Bijective by construction — no repeats, no misses.

void init_random_tracker() {
	if (!FLAGRANDOM) return;

	// If already initialized, free and re-init (range may have changed)
	if (g_feistel) {
		free(g_feistel);
		g_feistel = NULL;
	}

	g_feistel = (FeistelPermutation*) calloc(1, sizeof(FeistelPermutation));
	if (!g_feistel) {
		fprintf(stderr, "[E] Failed to allocate Feistel permutation\n");
		exit(EXIT_FAILURE);
	}

	// Compute domain size = number of CHUNKS (not individual keys).
	// Each chunk covers N_SEQUENTIAL_MAX keys, so:
	//   num_chunks = ceil(range / N_SEQUENTIAL_MAX)
	// The Feistel permutes chunk indices [0, num_chunks) so that every
	// non-overlapping chunk is visited exactly once in random order.
	Int range_size;
	range_size.Set(&n_range_end);
	range_size.Sub(&n_range_start);

	// Get raw range as uint64_t
	uint64_t raw_range = range_size.GetInt64();
	if (raw_range == 0) {
		// Range > 2^64 — compute num_chunks differently
		// For very large ranges, divide by N_SEQUENTIAL_MAX using Int math
		Int chunk_count;
		chunk_count.Set(&range_size);
		Int seq_max;
		seq_max.SetInt64(N_SEQUENTIAL_MAX);
		chunk_count.Div(&seq_max);
		if (!chunk_count.IsZero()) {
			g_feistel->domain_size = chunk_count.GetInt64();
		}
		if (g_feistel->domain_size == 0) {
			g_feistel->domain_size = 0x7FFFFFFFFFFFFFFFULL;
		}
		printf("[!] Range exceeds 64-bit; Feistel covers %" PRIu64 " chunks\n",
			g_feistel->domain_size);
	} else {
		g_feistel->domain_size = (raw_range + N_SEQUENTIAL_MAX - 1) / N_SEQUENTIAL_MAX;
	}

	// Calculate bit widths for the Feistel halves
	g_feistel->total_bits = 0;
	uint64_t temp = g_feistel->domain_size - 1;
	while (temp > 0) { g_feistel->total_bits++; temp >>= 1; }
	if (g_feistel->total_bits < 4) g_feistel->total_bits = 4;  // Minimum

	g_feistel->half_bits_R = g_feistel->total_bits / 2;
	g_feistel->half_bits_L = g_feistel->total_bits - g_feistel->half_bits_R;
	g_feistel->mask_L = (1ULL << g_feistel->half_bits_L) - 1;
	g_feistel->mask_R = (1ULL << g_feistel->half_bits_R) - 1;
	g_feistel->num_rounds = 8;  // 8 rounds for good diffusion

	g_feistel->counter = 0;
	g_feistel->exhausted = false;

	// Seed round keys from /dev/urandom (or rand on Windows)
#if defined(_WIN64) && !defined(__CYGWIN__)
	srand((unsigned)time(NULL));
	for (int i = 0; i < 8; i++) {
		g_feistel->round_keys[i] = ((uint64_t)rand() << 32) | rand();
	}
	feistel_mutex_handle = CreateMutex(NULL, FALSE, NULL);
#else
	FILE *urandom = fopen("/dev/urandom", "rb");
	if (urandom) {
		size_t rd = fread(g_feistel->round_keys, sizeof(uint64_t), 8, urandom);
		fclose(urandom);
		if (rd < 8) {
			for (int i = 0; i < 8; i++)
				g_feistel->round_keys[i] = (uint64_t)time(NULL) * 6364136223846793005ULL + i;
		}
	} else {
		for (int i = 0; i < 8; i++)
			g_feistel->round_keys[i] = (uint64_t)time(NULL) * 6364136223846793005ULL + i;
	}
	pthread_mutex_init(&feistel_mutex_handle, NULL);
#endif

	printf("[+] Feistel permutation initialized\n");
	printf("[+] Domain size: %llu keys (%d bits)\n",
		(unsigned long long)g_feistel->domain_size, g_feistel->total_bits);
	printf("[+] Every key will be checked exactly once in pseudo-random order\n");
	printf("[+] Memory: O(1) — no tracking table needed\n");
}
...
void cleanup_random_tracker() {
	if (g_feistel) {
		...
		g_feistel = NULL;
#if defined(_WIN64) && !defined(__CYGWIN__)
		CloseHandle(feistel_mutex_handle);
#else
		pthread_mutex_destroy(&feistel_mutex_handle);
#endif
	}
}

bool generate_unique_random_key(Int &key, const Int &min_range, const Int &max_range) {
	(void)max_range; // Domain size is pre-computed in g_feistel

	if (!g_feistel || g_feistel->exhausted) return false;

	uint64_t index;

	// Atomically grab next counter value
#if defined(_WIN64) && !defined(__CYGWIN__)
	WaitForSingleObject(feistel_mutex_handle, INFINITE);
#else
	pthread_mutex_lock(&feistel_mutex_handle);
#endif

	if (g_feistel->counter >= g_feistel->domain_size) {
		g_feistel->exhausted = true;
#if defined(_WIN64) && !defined(__CYGWIN__)
		ReleaseMutex(feistel_mutex_handle);
#else
		pthread_mutex_unlock(&feistel_mutex_handle);
#endif
		printf("\n[+] Range fully exhausted — every key has been checked.\n");
		return false;
	}

	index = g_feistel->counter;
	g_feistel->counter++;
	...
	// Map sequential counter → pseudo-random CHUNK INDEX via Feistel bijection
	uint64_t permuted_chunk = feistel_permute(g_feistel, index);

	// Convert chunk index to starting key: key = min_range + chunk * N_SEQUENTIAL_MAX
	Int min_copy;
	min_copy.Set((Int*)&min_range);
	key.Set(&min_copy);
	Int offset;
	offset.SetInt64(permuted_chunk);
	Int seq_max;
	seq_max.SetInt64(N_SEQUENTIAL_MAX);
	offset.Mult(&seq_max);
	key.Add(&offset);
